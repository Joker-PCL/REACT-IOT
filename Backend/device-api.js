...
const eventTimestamp = (machineID, table) => `INSERT IGNORE INTO ${table} (machineID, timestamp)
    SELECT ${machineID} AS machineID, TIMESTAMP(CONCAT(CURDATE(), ' 00:00:00')) + INTERVAL (t4*10000 + t3*1000 + t2*100 + t1*10 + t0) MINUTE
        ...
        db.query(
            ...
            function (err, list) {
                if (err) {
                    ...
                    const eventTimes = eventTimestamp(data.machineID, table);
